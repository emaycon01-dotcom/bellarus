...
const features = [
  {
    ...
    desc: "Crie documentos com qualidade profissional em segundos.",
  },
  {
    icon: Shield,
    title: "Segurança Avançada",
    desc: "Proteção de dados com criptografia de ponta a ponta.",
  },
  {
    icon: Zap,
    title: "Automação Total",
    desc: "Processos automatizados para máxima eficiência.",
  },
  {
    icon: Lock,
    title: "Controle de Acesso",
    desc: "Gerencie permissões e níveis de acesso facilmente.",
  },
];
        ...
        <div className="absolute top-6 left-0 right-0 overflow-hidden">
          ...
          >
            {Array.from({ length: 10 }).map((_, i) => (
              <span
                key={i}
                className="text-xs font-mono uppercase tracking-[0.3em] text-primary/20 select-none"
              >
                BELLARUS SISTEMAS • DOCUMENTOS DIGITAIS • SEGURANÇA •
              </span>
            ))}
          </motion.div>
        </div>

        {/* Bottom scrolling marquee (reversed) */}
        <div className="absolute bottom-6 left-0 right-0 overflow-hidden">
          ...
          >
            {Array.from({ length: 10 }).map((_, i) => (
              <span
                key={i}
                className="text-xs font-mono uppercase tracking-[0.3em] text-primary/15 select-none"
              >
                AUTOMAÇÃO • CONTROLE • QUALIDADE • INOVAÇÃO •
              </span>
            ))}
          </motion.div>
        </div>
        ...
        <div className="relative z-10 max-w-lg space-y-10">
          ...
          >
            <div className="flex items-center gap-4">
              <motion.div
                className="w-16 h-16 rounded-2xl flex items-center justify-center border border-primary/30 relative overflow-hidden"
                style={{
                  background: "linear-gradient(135deg, hsl(220 80% 15%), hsl(200 80% 35%))",
                }}
                animate={{ boxShadow: ["0 0 20px hsl(200 80% 55% / 0.2)", "0 0 40px hsl(200 80% 55% / 0.4)", "0 0 20px hsl(200 80% 55% / 0.2)"] }}
              ...
              <div>
                ...
                <p className="text-xs font-mono tracking-[0.25em] text-muted-foreground">
                  SISTEMAS
                </p>
              </div>
            </div>
        ...
        >
          ...
          <div className="lg:hidden flex flex-col items-center gap-3 mb-4">
            <motion.div
              ...
              animate={{ boxShadow: ["0 0 15px hsl(200 80% 55% / 0.2)", "0 0 30px hsl(200 80% 55% / 0.4)", "0 0 15px hsl(200 80% 55% / 0.2)"] }}
              transition={{ duration: 3, repeat: Infinity }}
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <motion.div whileTap={{ scale: 0.98 }}>
              <Button
                type="submit"
                className="w-full py-5 font-semibold text-sm relative overflow-hidden group"
                style={{
                  background: "linear-gradient(135deg, hsl(220 80% 15%), hsl(200 80% 35%))",
                }}
              >
