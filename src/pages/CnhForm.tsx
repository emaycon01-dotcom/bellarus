...
const CnhForm = () => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Scale factors from pixel coords (1653x2339) to PDF points
      ...
      // Helper: convert pixel coords to PDF coords (PDF y-axis is bottom-up)
      const drawText = (text: string, field: { top: number; left: number; w: number; h: number; fontSize: number }, options?: { color?: { r: number; g: number; b: number }; bold?: boolean; coverPlaceholder?: boolean }) => {
        ...
        // Cover placeholder text with white rectangle
        if (options?.coverPlaceholder !== false) {
          page.drawRectangle({
            x: pdfX,
            y: pdfY,
            width: pdfW,
      ...
      // Draw all text fields over placeholders
      ...
      // Generate QR code as PNG and embed
      ...
      try {
        ...
        qrCanvas.width = 520;
        qrCanvas.height = 520;
        // Create a temporary QR code element to capture
        ...
        await new Promise<void>((resolve) => {
          root.render(
            <QRCode value={verUrl} size={500} level="H" />
          );
      ...
      // Espelho vertical text (draw character by character)
      if (espelho) {
        const espFontSize = 8;
        ...
        // Cover placeholder
        page.drawRectangle({
          x: supField.left * scaleX - 2,
          y: pageH - (supField.top + supField.h) * scaleY,
          width: supField.w * scaleX + 4,
          height: supField.h * scaleY,
          color: rgb(1, 1, 1),
        });
        chars.forEach((ch, i) => {
          page.drawText(ch, {
        ...
        page.drawRectangle({
          x: infField.left * scaleX - 2,
          y: pageH - (infField.top + infField.h) * scaleY,
          width: infField.w * scaleX + 4,
          height: infField.h * scaleY,
          color: rgb(1, 1, 1),
        });
        chars.forEach((ch, i) => {
          page.drawText(ch, {
